...
VKRenderer::VKRenderer(Window* window_) : window(window_), instance(nullptr), debugMessenger(VK_NULL_HANDLE), physicalDevice(VK_NULL_HANDLE), device(VK_NULL_HANDLE), graphicsQueue(VK_NULL_HANDLE), presentQueue(VK_NULL_HANDLE), surface(VK_NULL_HANDLE), swapChain(VK_NULL_HANDLE), swapChainImages(), currentFrame(0), msaaSamples(VK_SAMPLE_COUNT_1_BIT){
	CreateInstance();
...
void VKRenderer::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels_, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory){
